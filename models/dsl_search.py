...
# Import certificate-guided pruning (CEGIS integration)
try:
    from trainers.certificates import mine_certificates, should_prune_hard, certificate_value_adjustment
except ImportError:
    mine_certificates = should_prune_hard = certificate_value_adjustment = None
    logging.warning("[DSL] Certificate module unavailable - proceeding without CEGIS pruning")
...
def score_candidate(candidate: BeamCandidate, demos: List[Tuple], priors: Dict, dsl_head: Any, cache: Optional[MemoizationCache] = None) -> float:
    ...
    try:
        ...
        # MDL PENALTY: Prefer shorter programs (critical for ARC-AGI-2)
        mdl_penalty = 0.02  # Configurable: 0.01-0.05 range
        program_length = len(candidate.ops)
        ...
        if candidate.is_template:
            ...
            score -= program_length * mdl_penalty
        else:
            # Standard penalty for longer programs (MDL prior)
            # Increased penalty to strongly bias toward simplicity (ARC-AGI-2 winners)
            score -= program_length * (mdl_penalty * 5.0)  # 0.10 default penalty
...
def enhanced_astar_search(demos, test, priors, dsl_head, template_library, cache, max_depth, beam_width, timeout_seconds, verbose, return_rule_info, op_bias=None):
    ...
    # === CEGIS: Mine certificates from demos for certificate-guided pruning ===
    certs = {}
    certs_pruned = 0
    if mine_certificates is not None and demos:
        try:
            certs = mine_certificates(demos)
            if certs and verbose:
                print(f"[CEGIS] Mined {len(certs)} certificates: {list(certs.keys())}")
        except Exception as e:
            if verbose:
                print(f"[CEGIS] Certificate mining failed: {e}")
            certs = {}
    ...
    while open_set and time.time() - start_time < timeout_seconds:
        ...
        for expansion in expansions[:beam_width]:  # Limit expansions
            ...
            try:
                ...
                # === CEGIS: Certificate-guided pruning ===
                if certs and should_prune_hard is not None:
                    try:
                        # Apply expansion to first demo and check certificates
                        test_result = expansion.apply(demos[0][0])
                        if should_prune_hard(certs, test_result, threshold=1.0):
                            certs_pruned += 1
                            if verbose and certs_pruned % 10 == 0:
                                print(f"[CEGIS] Pruned {certs_pruned} candidates (certificate violations)")
                            continue  # Skip this candidate - violates certificates

                        # Adjust score based on certificate penalties (soft pruning)
                        if certificate_value_adjustment is not None:
                            adjusted_score = certificate_value_adjustment(certs, test_result, score, penalty_weight=0.5)
                            expansion.score = adjusted_score
                    except Exception:
                        pass  # If pruning fails, allow candidate through
    ...
    if verbose:
        ...
        if certs:
            print(f"[CEGIS] Pruned {certs_pruned} candidates via certificate violations (search space reduction: {certs_pruned / max(1, iteration) * 100:.1f}%)")
        if template_miner:
...
def run_dsl_puct(demos, test, op_bias=None, max_depth=2, num_simulations=100, c_puct=1.4, return_rule_info=False, verbose=False, causal_context=None):
    ...
    PUCT combines value estimates with policy priors for principled explore/exploit trade-off
    (Silver et al., 2017 - AlphaGo Zero MCTS approach).

    Args:
        demos: List of (input, output) demonstration pairs
        test: Test input grid
        op_bias: Dictionary of operation priors (policy) from RelMem/ETS
        max_depth: Maximum program depth
        num_simulations: Number of MCTS simulations (higher = better but slower)
    ...
    """
    # Extract priors from demos
    priors = {}
    if demos:
        for demo_in, demo_out in demos:
            # Simple prior extraction based on color mapping
            if isinstance(demo_in, torch.Tensor) and isinstance(demo_out, torch.Tensor):
                in_colors = set(torch.unique(demo_in).tolist())
    ...
    result = beam_search(
        demos, test, priors,
        depth=max_depth,
        beam=max(3, num_simulations // 20),  # Convert simulations to beam width
        verbose=verbose,
        return_rule_info=return_rule_info,
        op_bias=op_bias or {}
    )
