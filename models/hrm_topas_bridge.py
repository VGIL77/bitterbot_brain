...
class HRMTOPASIntegrationConfig:
    ...
    dsl_ops_count: int = 41  # Number of DSL operations (from dsl_registry.py)
...
class HRMGuidedDSLSelector(nn.Module):
    ...
    def forward(self, z_H: torch.Tensor, z_L: torch.Tensor,
        ...
        # Pool confidence over sequence if needed (NeuroPlanner outputs [B, seq_len, ops])
        if confidence.dim() > 1:
            confidence = confidence.mean(dim=1)  # [B, seq_len] -> [B]
        should_halt = (q_halt > q_continue) | (confidence > self.config.adaptive_halting_threshold)

        # 🔒 GPU-FIRST: build scalars on the same device as z_H/q_halt (NOT CPU!)
        dev = z_H.device
        depth_now  = torch.full_like(q_halt, float(current_depth), device=dev)
        depth_next = torch.full_like(q_halt, float(current_depth + 1), device=dev)
        ...
        control_signals = {
            ...
            'adaptive_depth': torch.where(should_halt, depth_now, depth_next).clamp_(1, 12).to(torch.long)
        }
...
class PuzzleEmbeddingIntegrator(nn.Module):
    ...
    def __init__(self, config: HRMTOPASIntegrationConfig):
        ...
        # Projection from puzzle embedding to TOPAS width (conditional creation)
        self.puzzle_proj = nn.Linear(config.puzzle_emb_dim, config.topas_width) if config.puzzle_emb_dim > 0 else None
    ...
    def forward(self, grid_features: torch.Tensor,
        ...
        Args:
            ...
            puzzle_embedding: [B, puzzle_emb_dim] - HRM puzzle embedding (can be None if disabled)

        Returns:
            enhanced_features: [B, topas_width, H, W] - Enhanced grid features
        """
        # Handle disabled puzzle embeddings
        if self.puzzle_proj is None or puzzle_embedding is None or puzzle_embedding.numel() == 0:
            # No puzzle embedding enhancement - return original features
...
class HRMTOPASBridge(nn.Module):
    ...
    def forward(self,
                ...
                brain_priors: Optional[Dict[str, object]] = None) -> Dict[str, torch.Tensor]:
        """
        ...
        Args:
            ...
            brain_priors: Optional priors from BrainGraph (op_bias: dict, param_priors: dict[str,tensor], puzzle_embedding: tensor)
        ...
        # === DTYPE UNIFICATION (GRADIENT FLOW FIX #3) ===
        # Cast all inputs to float32 for consistency
        if grid_features.dtype != torch.float32:
        ...
        # Cast HRM states to float32
        if z_H.dtype != torch.float32:
            z_H = z_H.float()
        if z_L.dtype != torch.float32:
            z_L = z_L.float()

        if puzzle_embedding is not None and puzzle_embedding.dtype != torch.float32:
            puzzle_embedding = puzzle_embedding.float()
        ...
        # 6b. Merge brain graph priors (if provided)
        if brain_priors is not None:
            try:
                # Op priors: convert dict → logit addend
                bp = brain_priors.get("op_bias") or {}
                if isinstance(bp, dict) and len(bp):
                    # Add small constant logit bump for biased ops
                    # (caller should map op names to indices if needed)
                    bump = torch.zeros_like(dsl_op_logits)
                    # Heuristic: uniform bump across batch for seen ops
                    for b in range(bump.size(0)):
                        for i in range(min(self.config.dsl_ops_count, bump.size(1))):
                            if i < len(bp):
                                bump[b, i] += 0.25
                    dsl_op_logits = dsl_op_logits + bump

                # Param priors: override/insert
                bpp = brain_priors.get("param_priors") or {}
                if isinstance(bpp, dict):
                    for k, v in bpp.items():
                        if torch.is_tensor(v):
                            param_priors[k] = v.to(dsl_op_logits.device, dtype=torch.float32)

                # Puzzle embedding override if present and not already set
                bg_puzzle_emb = brain_priors.get("puzzle_embedding")
                if bg_puzzle_emb is not None and puzzle_embedding is None:
                    puzzle_embedding = bg_puzzle_emb
            except Exception:
                pass  # Graceful degradation if brain_priors malformed
    ...
    def extract_dsl_operation_dict(self, dsl_op_biases: torch.Tensor, 
        ...
        # GPU-FIRST: Keep on GPU, use .item() for scalar conversion
        biases = dsl_op_biases[0].detach()  # [dsl_ops_count]
        return {op: float(bias.item()) for op, bias in zip(dsl_ops_list, biases)}
    ...
    def compute_adaptive_search_params(self, control_signals: Dict[str, torch.Tensor],
        ...
        """
        # 🔒 GPU-FIRST: Use plain Python fallbacks (not CPU tensors!)
        should_halt    = control_signals.get('should_halt', False)
        confidence     = control_signals.get('confidence', 0.5)
        adaptive_depth = control_signals.get('adaptive_depth', base_depth)
    ...
    @torch.no_grad()
    def predict_value(self, topas_feat: torch.Tensor, z_H: torch.Tensor) -> torch.Tensor:
        """
        Predict EM-likelihood style value from global TOPAS features and HRM H-state.
        Returns [B] sigmoid(value_logit) on the correct device/dtype.
        """
        if topas_feat.dim() == 4:
            topas_global = torch.mean(topas_feat, dim=(2,3))
        else:
            topas_global = topas_feat
        x = torch.cat([topas_global, z_H.to(topas_global.dtype)], dim=-1)
        v = self.value_head(x).squeeze(-1)
        return v.sigmoid().clamp_(0,1)