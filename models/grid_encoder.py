...
class GridEncoder(HRMAwareGridEncoder):
    ...
    def forward(self, x: torch.Tensor, hrm_context: Optional[Dict[str, torch.Tensor]] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            x: [B, H, W] integer grid or various float/int formats
        ...
        """
        # === Normalize input to [B,width,H,W], robust to channels-last ===
        if x.ndim == 2:
            x = x.unsqueeze(0)                     # [H,W] -> [1,H,W]
        if x.ndim == 3:
            if x.dtype in (torch.long, torch.int32, torch.int64):
                x = x.clamp(0, 9)
                feat = self.embed(x)               # [B,H,W, width]
                feat = feat.permute(0, 3, 1, 2).contiguous()  # [B,width,H,W]
            else:
                x = x.unsqueeze(1)                 # [B,1,H,W]
                feat = x.to(torch.float32).expand(-1, self.width, -1, -1)
        elif x.ndim == 4:
            # Accept [B,1,H,W], [B,width,H,W], or channels-last [B,H,W,1/width]
            if x.shape[1] not in (1, self.width) and x.shape[-1] in (1, self.width):
                x = x.permute(0, 3, 1, 2).contiguous()        # channels-last -> channels-first
            if x.shape[1] == 1:
                feat = x.to(torch.float32).expand(-1, self.width, -1, -1)
            elif x.shape[1] == self.width:
                feat = x.to(torch.float32)
            elif x.dtype in (torch.long, torch.int32, torch.int64):
                # Probably [B,H,W,?] integers – use embedding path safely
                x = x.squeeze(1) if x.shape[1] == 1 else x
                x3 = x if x.ndim == 3 else x.permute(0,2,3,1)[:, :, :, 0]  # [B,H,W]
                x3 = x3.clamp(0,9).long()
                feat = self.embed(x3).permute(0,3,1,2).contiguous()
            else:
                raise ValueError(f"[GridEncoder] Unsupported 4D input shape {tuple(x.shape)}; "
                                 f"expected C in {{1,{self.width}}} or channels-last.")
        else:
            raise ValueError(f"[GridEncoder] Invalid input ndim={x.ndim}; expected 2D/3D/4D")

        # Shape sanity: catch HW*width leaks masquerading as channels
        B, C, H, W = feat.shape
        assert C == self.width, \
            f"[GridEncoder] Channel mismatch after normalization: got C={C}, expected {self.width}. " \
            f"Shape={tuple(feat.shape)}; if C≈H*W*width you likely reshaped incorrectly upstream."
