...
class TemplateLibrary:
    ...
    def _sigmoid(x: float) -> float:
        """Sigmoid helper for score normalization"""
        try:
            import math
    ...
    def get_templates_by_overlap(self, op_weights: dict, max_count: int = 5) -> List[Template]:
        """
        Context-aware ranking: score template by how much its ops align with
        the *current* episodic op-weights. Combines MDL (t.score) and overlap.

        Args:
            op_weights: Current operation weights from episodic memory
            max_count: Maximum templates to return

        Returns:
            List of templates ranked by: 60% context overlap + 40% MDL score
        """
        scored = []
        if not self.templates or not isinstance(op_weights, dict):
            return self.get_templates_by_score(max_count=max_count)

        for tpl in self.templates.values():
            if not hasattr(tpl, "ops") or not tpl.ops:
                continue

            # Compute overlap with current episodic op-weights
            overlap = 0.0
            for op in tpl.ops:
                # Extract op name (handle both string and tuple formats)
                op_name = op[0] if isinstance(op, tuple) else op
                overlap += float(op_weights.get(op_name, 0.0))

            overlap = overlap / max(1, len(tpl.ops))  # Normalize by template length

            # Combine overlap with MDL score
            mdlf = self._sigmoid(getattr(tpl, "score", 0.0))
            # 60% context, 40% MDL
            final = 0.6 * overlap + 0.4 * mdlf

            scored.append((final, tpl, overlap))

        scored.sort(key=lambda x: -x[0])

        # Annotate templates with overlap for logging
        out = []
        for _, tpl, ov in scored[:max_count]:
            tpl.context_overlap = ov
            out.append(tpl)
...
class WormholeTemplateMiner:
    ...
    def num_templates(self) -> int:
        return len(getattr(self, "templates", []))

    def is_ready(self, programs) -> bool:
        """Ready only if we have real programs to mine (no partial/dummy)."""
        return bool(programs) or self.num_templates() >= 3

    def consume_repair_ops(self, ops, score=0.5):
        # Treat near-miss repairs as weak templates
        if not hasattr(self, "templates"):
            self.templates = []
        if ops:
            self.templates.append({"ops": list(ops), "score": float(score)})
    ...
    def mine_from_programs(self, programs, top_k: int = 5, min_support: int = 1, ctx=None):
        """
        Public wrapper with structured diagnostics.
        Never crashes; always emits starvation/empty signals for conductor.

        Args:
            programs: List of programs to mine from
            top_k: Number of top templates to return
            min_support: Minimum support for template (lowered from 2 to 1 for earlier mining)
            ctx: Optional context dict for metrics
        """
        if not programs:
            if ctx:
                ctx.setdefault('signals', {})['wormhole_ready'] = False
                ctx.setdefault('metrics', {})['wormhole_starvation'] = ctx['metrics'].get('wormhole_starvation', 0) + 1
            self.logger.error('[Wormhole] STARVATION – no programs provided')
            raise RuntimeError('[STRICT] Wormhole starvation – upstream rail failed')

        try:
            templates, diag = self.mine_from_programs_diag(programs, top_k=top_k, min_support=min_support)
        except Exception as e:
            if ctx:
                ctx.setdefault('signals', {})['wormhole_error'] = True
                ctx.setdefault('metrics', {})['wormhole_errors'] = ctx['metrics'].get('wormhole_errors', 0) + 1
            self.logger.error(f"[Wormhole] mining exception: {e}")
            return []

        if not templates:
            if ctx:
                ctx.setdefault('signals', {})['wormhole_empty'] = True
                ctx.setdefault('metrics', {})['wormhole_failures'] = ctx['metrics'].get('wormhole_failures', 0) + 1
                ctx.setdefault('diagnostics', {})['wormhole_last_rejection'] = diag.get("rejected_reasons", {})
            diag_summary = {
                "input_count": diag.get("input_count"),
                "candidates_generated": diag.get("candidates_generated"),
                "accepted": diag.get("accepted"),
                "top_rejections": dict(list(diag.get("rejected_reasons", {}).items())[:5])
            }
            self.logger.debug(f"[Wormhole] empty result; diag={diag_summary}")
            return []

        if ctx:
            ctx.setdefault('signals', {})['wormhole_ready'] = True
            ctx.setdefault('metrics', {})['wormhole_success'] = ctx['metrics'].get('wormhole_success', 0) + 1

        return [tuple(tpl) for tpl in templates[:top_k]]

    def mine_from_programs_diag(self, programs, top_k: int = 5, min_support: Optional[int] = None,
                                min_length: Optional[int] = None, allow_partial: bool = False,
                                verbose: bool = False) -> Tuple[List[List[str]], Dict[str, Any]]:
        """
        Diagnostic miner: returns (templates, diagnostics)
        - templates: list of templates (each a list of op names)
        - diagnostics: dict containing counts and rejection reasons
        This method does NOT perform any silent fallbacks; it gives you the info to fix upstream.
        """
        diagnostics: Dict[str, Any] = {
            ...
            "filters": {
                "min_support": min_support if min_support is not None else getattr(self.consolidator, "min_support", None),
        ...
        try:
            # Normalize programs into list-of-programs where each program is list-of-opnames OR list-of-(op,params)
            if programs is None:
                diagnostics["errors"].append("programs is None")
                return [], diagnostics
            if isinstance(programs, dict):
                programs = programs.get("programs", [])
            if not isinstance(programs, (list, tuple)):
                diagnostics["errors"].append(f"expected programs list, got {type(programs)}")
                return [], diagnostics

            diagnostics["input_count"] = len(programs)
            diagnostics["program_samples"] = programs[:3]

            if len(programs) == 0:
                diagnostics["errors"].append("no programs provided")
                return [], diagnostics

            # Convert each program to a list of op names for candidate extraction
            normalized_programs: List[List[str]] = []
            for prog in programs:
                try:
                    if prog is None:
                        raise ValueError("null program")
                    if isinstance(prog, dict):
                        # common upstream shape: {'ops': [...]} or {'ops_used': [...]}
                        ops = prog.get("ops") or prog.get("ops_used") or prog.get("programs")
                        if ops is None:
                            # if wrapped structure contains rule_info maybe
                            ops = prog.get("rule_info", {}).get("ops") or prog.get("rule_info", {}).get("ops_used")
                        if ops is None:
                            raise ValueError("unreadable program dict")
                    elif isinstance(prog, (list, tuple)):
                        # could be list of (op, params) tuples or list of strings
                        if len(prog) > 0 and isinstance(prog[0], tuple):
                            ops = [op for op, _ in prog]
                        else:
                            ops = list(prog)
                    else:
                        # try attribute access
                        ops = getattr(prog, "ops", None) or getattr(prog, "ops_used", None)
                        if ops is None:
                            raise ValueError(f"unreadable program type: {type(prog)}")

                    if not ops or not isinstance(ops, (list, tuple)):
                        raise ValueError("ops empty or wrong type")

                    normalized_programs.append(list(ops))
                except Exception as e:
                    diagnostics["rejected_reasons"].setdefault("unreadable_program", 0)
                    diagnostics["rejected_reasons"]["unreadable_program"] += 1
                    diagnostics["errors"].append(str(e))
                    continue

            if len(normalized_programs) == 0:
                diagnostics["errors"].append("no readable programs after normalization")
                return [], diagnostics

            # candidate extraction: subsequences of length 2..max_len (configurable)
            candidates = []
            max_pattern_len = 4
            for prog_idx, ops in enumerate(normalized_programs):
                L = len(ops)
                for start in range(0, L):
                    for end in range(start + 2, min(start + max_pattern_len + 1, L + 1)):
                        cand = tuple(ops[start:end])
                        candidates.append({"cand": cand, "source_idx": prog_idx, "source_len": L})

            diagnostics["candidates_generated"] = len(candidates)

            # Compute support for each candidate efficiently
            support_counter = Counter()
            for c in candidates:
                support_counter[c["cand"]] += 1

            # Determine thresholds
            ms = min_support if min_support is not None else getattr(self.consolidator, "min_support", 2)
            ml = min_length if min_length is not None else getattr(self, "consolidator", "min_length") if hasattr(self, "consolidator") else 2
            if ml is None:
                ml = 2

            # Filter candidates with explicit reasons
            filtered = []
            for cand, count in support_counter.items():
                reason = None
                if len(cand) < ml:
                    reason = f"shorter_than_min_length({len(cand)}<{ml})"
                elif count < ms:
                    reason = f"support_too_small({count}<{ms})"
                elif not allow_partial and any(op in ("NOOP", "UNK") for op in cand):
                    reason = "contains_forbidden_op"
                if reason is None:
                    filtered.append((cand, count))
                else:
                    diagnostics["rejected_reasons"].setdefault(reason, 0)
                    diagnostics["rejected_reasons"][reason] += 1

            # Build templates from filtered and rank them
            # Keep top_k by support then length
            filtered_sorted = sorted(filtered, key=lambda x: (-x[1], -len(x[0])))
            templates = [list(cand) for cand, cnt in filtered_sorted[:top_k]]
            diagnostics["accepted"] = len(templates)

            return templates, diagnostics

        except Exception as e:
            diagnostics["errors"].append(str(e))
            self.logger.exception("mine_from_programs_diag internal error")
            return [], diagnostics

    def emit_signals(self, ctx=None):
        """Emit structured signals about Wormhole template mining status for conductor integration"""
        if ctx is not None:
            signals = ctx.setdefault('signals', {})
            metrics = ctx.setdefault('metrics', {})

            # Core readiness and template status
            template_count = self.num_templates()
            signals['wormhole_ready'] = template_count >= 3
            metrics['wormhole_template_count'] = template_count

            # Library status
            if hasattr(self, 'library') and self.library:
                library_size = len(self.library)
                metrics['wormhole_library_size'] = library_size
            else:
                metrics['wormhole_library_size'] = 0

            # TTL and decay information
            if hasattr(self, 'ttl_decay') and self.ttl_decay:
                active_templates = sum(1 for ttl in self.ttl_decay.values() if ttl > 0)
                metrics['wormhole_active_templates'] = active_templates
                if self.ttl_decay.values():
                    metrics['wormhole_avg_ttl'] = sum(self.ttl_decay.values()) / len(self.ttl_decay)
                else:
                    metrics['wormhole_avg_ttl'] = 0.0
            else:
                metrics['wormhole_active_templates'] = 0
                metrics['wormhole_avg_ttl'] = 0.0

            # Consolidator status
            if hasattr(self, 'consolidator') and self.consolidator:
                signals['wormhole_consolidator_available'] = True
                if hasattr(self.consolidator, 'library'):
                    metrics['wormhole_consolidator_templates'] = len(self.consolidator.library)
                else:
                    metrics['wormhole_consolidator_templates'] = 0
            else:
                signals['wormhole_consolidator_available'] = False
                metrics['wormhole_consolidator_templates'] = 0

            # Mining capability signals
            signals['wormhole_can_mine'] = template_count > 0 or (hasattr(self, 'consolidator') and self.consolidator)

            # Starvation detection - wormhole is starving if no templates and no consolidator
            if template_count == 0 and not signals.get('wormhole_consolidator_available', False):
                metrics['wormhole_starvation'] = metrics.get('wormhole_starvation', 0) + 1
