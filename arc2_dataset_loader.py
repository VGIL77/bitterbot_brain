...
class ARC2Dataset(Dataset):
    ...
    def __init__(self, challenge_file: str, solution_file: Optional[str] = None,
                 device: str = "cuda", max_grid_size: int = 30, min_test_cases: int = 0):
        self.device = device
        ...
        # Handle both single JSON file and directory structure
        if os.path.isfile(challenge_file):
            with open(challenge_file, "r") as f:
                self.challenges = json.load(f)
        elif os.path.isdir(challenge_file):
            # Load from directory structure (individual JSON files)
            self.challenges = {}
            for json_file in os.listdir(challenge_file):
                if json_file.endswith('.json'):
                    task_id = json_file[:-5]  # Remove .json extension
                    with open(os.path.join(challenge_file, json_file), "r") as f:
                        self.challenges[task_id] = json.load(f)
        else:
            raise ValueError(f"Invalid challenge_file: {challenge_file} (not a file or directory)")
    ...
    def __getitem__(self, idx: int):
        # Bounded retry loop to avoid recursion
        for attempt in range(16):
            ...
            # Passed curriculum check or not in warmup - process task
            break
        ...
        test_inputs = []
        test_outputs_from_challenge = []
        has_outputs_in_challenge = False
        ...
        for t in task["test"]:
            ...
            # Check if output exists in challenge file itself
            if "output" in t:
                out = torch.tensor(_sanitize_grid(t["output"], max_size=self.max_grid_size),
                                   dtype=torch.long, device=self.device).unsqueeze(0)
                test_outputs_from_challenge.append(out)
                has_outputs_in_challenge = True

        # Use outputs from challenge file if available, otherwise check solutions file
        test_outputs = None
        if has_outputs_in_challenge:
            test_outputs = test_outputs_from_challenge
        elif self.solutions and tid in self.solutions:
        ...
        # ARC-II: Calculate true sequence length from first test input
        # This drives NeuroPlanner's dynamic seq_len allocation
        if test_inputs and len(test_inputs) > 0:
            first_test = test_inputs[0]
            if first_test.dim() == 3:  # [1, H, W]
                H, W = first_test.shape[1], first_test.shape[2]
            else:  # [H, W]
                H, W = first_test.shape[0], first_test.shape[1]
            seq_len = H * W
        # seq_len removed - NeuroPlanner computes it dynamically from actual grid size
