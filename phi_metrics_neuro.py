...
def phi_synergy_features(tokens: torch.Tensor, parts: int = 2) -> torch.Tensor:
    ...
    if D % parts != 0:
        raise RuntimeError(f"[StrictPhi] Feature dimension {D} not divisible by {parts}")
    ...
    try:
        ...
        raise RuntimeError(f"[StrictPhi] synergy computation failed: {e}")
...
def kappa_floor(tokens: torch.Tensor, H: int, W: int) -> torch.Tensor:
    ...
    try:
        ...
        raise RuntimeError(f"[StrictPhi] kappa floor computation failed: {e}")
...
def cge_boost(tokens: torch.Tensor, phi_before: Optional[torch.Tensor] = None, 
    ...
    try:
        ...
        raise RuntimeError(f"[StrictPhi] CGE boost computation failed: {e}")
